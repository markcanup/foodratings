...
export default function MainPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const [
        ...
        { data: cuisineData, error: cuisineError }
      ] = await Promise.all([
        supabase
          .from('restaurants')
          .select(`
            *,
            dishes (
              *,
              ratings (
                *,
                users:users!ratings_user_id_fkey (id, name)
              )
            )
          `),
      ...
      if (restError) {
        alert('Error loading restaurants: ' + restError.message)
        return
      }

      if (userError) {
        alert('Error loading users: ' + userError.message)
        return
      }

      if (cuisineError) {
        alert('Error loading cuisines: ' + cuisineError.message)
        return
      }
  ...
  const getRestaurantDishes = (restaurant) => restaurant.dishes || []

  const getRestaurantRatings = (restaurant) =>
    getRestaurantDishes(restaurant).flatMap(dish => dish.ratings || [])

  const getAverageRating = (restaurant) => {
    const restaurantRatings = getRestaurantRatings(restaurant).filter(r => typeof r.rating === 'number')
  ...
  const getMostRecentRatingDate = (restaurant) => {
    const dates = getRestaurantRatings(restaurant)
      .map(r => r.date_rated)
      .filter(Boolean)
  ...
  const getRatingsBreakdown = (restaurant) => {
    const breakdown = {}
    users.forEach(u => { breakdown[u.id] = {} })
    getRestaurantRatings(restaurant).forEach(r => {
      const personRatings = breakdown[r.users?.id]
      if (personRatings && typeof r.rating === 'number') {
  ...
  const filteredRestaurants = restaurants
    .filter(r => {
      ...
      const dishesInRest = getRestaurantDishes(r)
      if (dishFilter && !dishesInRest.some(d => d.name.toLowerCase().includes(dishFilter.toLowerCase()))) return false
      if (selectedUserId) {
        const hasMatchingRating = dishesInRest.some(dish =>
          (dish.ratings || []).some(rt => rt.user_id === selectedUserId)
        )
    ...
    .sort((a, b) => {
      ...
      const dateA = new Date(getMostRecentRatingDate(a) || 0)
      const dateB = new Date(getMostRecentRatingDate(b) || 0)
    ...
    <div className="top-controls">
      ...
      <ul>
        {filteredRestaurants.map(r => {
          const average = getAverageRating(r)
          const mostRecent = getMostRecentRatingDate(r)
          const breakdown = getRatingsBreakdown(r)
          const restaurantDishes = getRestaurantDishes(r)
